...
# pipeline
text_clf = Pipeline([('vect', CountVectorizer(ngram_range=(1, 2),token_pattern=r'\b\w+\b', min_df=1)),('tfidf', TfidfTransformer(sublinear_tf=True)),('svm', OneVsRestClassifier(LinearSVC(C=0.1, random_state=0))),])
print('get pipeline')
print('training')
...
test_df_gold.to_csv('bigram_gold.csv',index=False)
test_df_pred.to_csv('bigram_pred.csv',index=False)
