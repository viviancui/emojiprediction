...
#######
word2vec_model = gensim.models.Word2Vec.load('external_word2vec.model')
...
def clean_text(text_df):
	...
	no_stop['word2vec'] = no_stop['text_list_no_stop'].apply(lambda x:transform(x))
...
train_df = ready_df[['word2vec','target']]
...
test_df = test_ready_df[['word2vec','target']]
...
# text_clf = Pipeline([('svm', OneVsRestClassifier(SVC(C=0.1, random_state=0)))])
text_clf = OneVsRestClassifier(LinearSVC(C=0.1, random_state=0))
print('got pipeline')
print('start training')
X = train_df.word2vec.tolist()
y = train_df.target
text_clf.fit(X, y)
predicted = text_clf.predict(test_df.word2vec.tolist())
...
mat.dump("external_w2v_matrix.dat")
# mat2 = numpy.load("my_matrix.dat")

test_df_gold = test_df[['target']]
test_df_pred = test_df[['predicted']]
test_df_gold.to_csv('external_w2v_gold.csv',index=False)
test_df_pred.to_csv('external_w2v_pred.csv',index=False)

test_df_new = test_df[['predicted','target']]
test_df_new.to_csv('external_w2v_result.csv',index=False)
